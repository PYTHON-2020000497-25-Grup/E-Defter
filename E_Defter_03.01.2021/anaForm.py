...
class AnaForm(QtWidgets.QMainWindow):
    ...
    def tableLoad(self):
        ...
        for i in self.data:
            ...
            self.ui.tableWSinifListele.setCellWidget(rowIndex, 4, checkBox)
            rowIndex += 1
    ...
    def EKLE(self):
        ...
        checked_list = []
        for i in range(self.ui.tableWSinifListele.rowCount()):
            
         
            if self.ui.tableWSinifListele.cellWidget(i, 4).isChecked():
                no = self.ui.tableWSinifListele.item(i,0).text()
                checked_list.append(no)
                listDevamsız = ' '.join([str(elem) for elem in checked_list])
        ...
        if sinif and ogrAdi and dersAdi and konuKazanim :
            
            curs.execute("INSERT INTO Dersler \
                     (Sinif,Ogradi,Dersadi,Konukazanim,Tarih,Devamsız_No) \
                      VALUES (?,?,?,?,?,?)", \
                         (sinif,ogrAdi,dersAdi,konuKazanim,tarih,listDevamsız))
            conn.commit()
            print("Kayıt Başarılı")
        
            QMessageBox.about(self, "e-defter-DATABASE", "Kayıt işlemi başarılı..")
        else:
            QMessageBox.about(self, "e-defter-DATABASE", "Lütfen tüm bilgileri giriniz")
